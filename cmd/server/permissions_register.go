...
import (
	...
	"iag-ai-platform/backend/internal/models"
)

func registerPermissionsLoop(ctx context.Context, cfg config.Config) {
	if cfg.ServiceClientSecret == "" {
		log.Printf("ai-platform: SERVICE_CLIENT_SECRET unset — skipping permissions registration")
		return
	}
	...
	for {
		...
		err := platformserviceauth.RegisterPermissions(regCtx, saClient, cfg.JWTIssuer, "ai-platform", perms)
		cancel()
		if err == nil {
			log.Printf("ai-platform: registered %d permissions", len(perms))
			return
		}
		log.Printf("ai-platform: permissions register failed: %v (retry in %s)", err, backoff)
		select {
		case <-ctx.Done():
			return
